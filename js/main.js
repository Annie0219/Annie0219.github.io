// --- TYPING EFFECT: "I [verb] robust intelligence." ---
...
// highlight color when verb finishes typing
const highlightColor = "#b30000"; // MIT crimson
let i = 0;        // current verb index
let j = 0;        // current letter index
let deleting = false;
let delay = 0;

function tick() {
  const fullVerb = verbs[i];
  const prefix = "I&nbsp;";
  const suffix = "&nbsp;robust&nbsp;intelligence.";

  // dynamic typing only affects the verb
  const currentVerb = fullVerb.slice(0, j);
  el.innerHTML = `${prefix}<span id="verb">${currentVerb}</span>${suffix}`;

  const verbSpan = document.getElementById("verb");

  if (!deleting && j < fullVerb.length) {
    j++;
  } else if (deleting && j > 0) {
    j--;
  } else if (!deleting && j === fullVerb.length) {
    ...
    delay = 3000; // pause before deleting
    deleting = true;
  } else if (deleting && j === 0) {
    deleting = false;
    i = (i + 1) % verbs.length;
...
new IntersectionObserver(([entry]) => {
  if (entry.isIntersecting) nav.classList.remove('solid');
